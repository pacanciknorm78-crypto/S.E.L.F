...
def stats():
    ...
    print(ac)
    print(division)
    print(f"╟{" Статистика ":─^{Sizes.header}}╢")
    print(f"║{' """ Реплика фамильяра """    ':>{Sizes.header}}║") #Что-то в духе: "Смотри, как продвинулся"
    # print(f"║ {"Дата первого входа:":<{39}} {str(data.userdata["firstDayUserAppeared"]):<{59}}║")
    # print(f"║ {"Текущая длина серии:":<{39}} {str(data.userdata["currentStreakLength"]):<{59}}║")
    # print(f"║ {"Максимальная длина серии:":<{39}} {str(data.userdata["maxStreakLength"]):<{59}}║")
    # print(f"║ {"Общее колличество срывов серии:":<{39}} {str(data.userdata["amountOfLoseStreaks"]):<{59}}║")
    print(f"╟{"":─^{Sizes.header}}╢")
    print(f"║{" Назад в Лагерь - 1 | Характеристики героя - 2 ":^{Sizes.header}}║")
    print(end)
def characteristics():
    clear()
    print(head)
    ...
    print("заглушка")
    print(f"╟{"":─^{Sizes.header}}╢")
    print(f"║{" Назад в Лагерь - 1 | Статистика - 2 ":^{Sizes.header}}║")
    print(end)
...
def familyar():
    ...
    print(ac)
    print(division)
    print(f"╟{" Информация о фамильяре ":─^{Sizes.header}}╢")
    print(f"║{' """ Реплика фамильяра """    ':>{Sizes.header}}║") # Хочешь узнать что то обо мне?
    print(f"║ {"Полное имя: ПОЛНОЕ ИМЯ ФАМИЛЬЯРА":<{30}} {"Пол: Муж/жен":<{65}} ║")
    print(f"║ {'Возраст: Цифрой':<{32}} {"Рост: Цифрой":<{31}}  {"Вес: Цифрой":{32}} ║")
    print(f"║ {'Список хобби: Список хобби через запятую 3-4 штучки':<{Sizes.header-2}} ║")
    print(f"║ {'Любимое блюдо: одно или два реальных блюда':<{Sizes.header-2}} ║")
    print(f"║ {'День рождения: День, месяц, мб год':<{50}} {"Знак зодиака: Название":<{47}} ║")
    print(f"║ {'Место рождения: Место':<{Sizes.header-2}} ║")
    print(f"║ {'Уровень доверия: уровень доверия':<{Sizes.header-2}} ║")
    print(f"╟{"":─^{Sizes.header}}╢")
    print(f"║{" В работе ":^{Sizes.header}}║")
    print(end)
def bill_board():
    clear()
    print(head)
    print(ac)
    print(division)
    print(f"╟{" Доска Объявлений ":─^{Sizes.header}}╢")
    print(f"║{' """ Реплика фамильяра """    ':>{Sizes.header}}║") # Похоже тут етсь задачи
    print(f"║ {"------ QUEST GIVER ":-<{Sizes.header-1}}║")
    print(f"║ {"  LATER HERE, WILL BE SHOWN SOME QUEST TEXT, USING F-STRINGS AND MAYBE IT":<{Sizes.header-1}}║")
    print(f"║ {"  WILL BE LIKE TWO LINES OF THIS TEXT.":─<{Sizes.header-1}}║")
    print(f"║ {"NUM"} {" NEED: SHORT QUEST TEXT. ":<{56}}{"COMPLETED/NOT COMPLETED  ":>{39}}║")
    print(f"║ {"":-<{Sizes.header-1}}║")
    print(f"║ {"------ QUEST GIVER ":-<{Sizes.header-1}}║")
    print(f"║ {"  LATER HERE, WILL BE SHOWN SOME QUEST TEXT, USING F-STRINGS AND MAYBE IT":<{Sizes.header-1}}║")
    print(f"║ {"  WILL BE LIKE TWO LINES OF THIS TEXT.":─<{Sizes.header-1}}║")
    print(f"║ {"NUM"} {" NEED: SHORT QUEST TEXT. ":<{56}}{"COMPLETED/NOT COMPLETED  ":>{39}}║")
    ...
    print(f"║ {"":-<{Sizes.header-1}}║")
    print(f"╟{"":─^{Sizes.header}}╢")
    print(f"║{" Любой ввод для продолжения ":^{Sizes.header}}║")
    print(end)
...
def equipment():
    ...
    print(ac)
    print(division)
    print(f"╟{" Снаряжение ":─^{Sizes.header}}╢")
    print(f"║{' """ Реплика фамильяра """    ':>{Sizes.header}}║")  # Похоже тут наши вещи
    print(f"║ {"В снаряжении":^{Sizes.header//2-2}} │ {"В сумках":^{Sizes.header//2-3}} ║")
    print(f"║ {"Именные часы":<{Sizes.header//2-2}} │ {"Изумруд: 7":<{Sizes.header//2-3}} ║")
    print(f"║ {"Одноручный меч":<{Sizes.header//2-2}} │ {"Артефакт: 1":<{Sizes.header//2-3}} ║")
    print(f"║ {"Небольшой поясной мешок":<{Sizes.header//2-2}} │ {"Монеты: 234":<{Sizes.header//2-3}} ║")
    print(f"║ {"":<{Sizes.header//2-2}} │ {"Рыба: 12":<{Sizes.header//2-3}} ║")
    print(f"╟{"":─^{Sizes.header}}╢")
    print(f"║{" Любой ввод для продолжения ":^{Sizes.header}}║")
    print(end)
...
def world_map():
    ...
    print(ac)
    print(division)
    print(f"╟{" Мировая карта ":─^{Sizes.header}}╢")
    print(f"║{' """ Реплика фамильяра """    ':>{Sizes.header}}║")  # Похоже тут мы можем узнать маршрут до люой точки
    print(f"║ {"   Руины   ":-^{Sizes.header-2}} ║")
    print(f"║ {"Камни, проросшие мхом, хранят лишь эхо прошлого.":<{Sizes.header-2}} ║")
    print(f"║ {" Здесь нечем поживиться, но тишина здесь особенная.":<{Sizes.header-2}} ║")
    print(f"║ {"   Повозка торговца   ":-^{Sizes.header-2}} ║")
    print(f"║ {"Скрипучая повозка, набитая диковинками. Торговец щурится хитро: цены кусаются,":<{Sizes.header-2}} ║")
    print(f"║ {"но без него в этих землях — никуда.":<{Sizes.header-2}} ║")
    print(f"║ {"   Озеро   ":-^{Sizes.header-2}} ║")
    print(f"║ {"Вода гладкая, как зеркало. Здесь можно забыться у берега с удочкой.":<{Sizes.header-2}} ║")
    print(f"║ {"Говорят, в глубине живут те, кто забирает усталость.":<{Sizes.header-2}} ║")
    print(f"║ {"   Пещера   ":-^{Sizes.header-2}} ║")
    print(f"║ {"Зев пещеры чернеет угрожающе. Внутри сыро и темно, но трусливых тянет сюда против воли.":<{Sizes.header-2}} ║")
    print(f"║ {"Страх здесь — лишь иллюзия.":<{Sizes.header-2}} ║")
    print(f"║ {"   Поляна   ":-^{Sizes.header-2}} ║")
    print(f"║ {"Залитое солнцем место. Здесь даже воздух кажется теплее, а трава мягче.":<{Sizes.header-2}} ║")
    print(f"║ {"Фамильяры любят сидеть здесь в тишине.":<{Sizes.header-2}} ║")
    print(f"║ {"   Землянка   ":-^{Sizes.header-2}} ║")
    print(f"║ {"Низкая дверь ведёт в жилище старого дварфа.":<{Sizes.header-2}} ║")
    print(f"║ {"Он нелюдим, но если ты пришёл не за подачками, то уйдёшь с полезной вещью.":<{Sizes.header-2}} ║")
    print(f"║ {"   Башня Вознесения   ":-^{Sizes.header-2}} ║")
    print(f"║ {"Высокая башня, где каждый этаж — проверка на прочность. Здесь ты сражаешься не с монстрами,":<{Sizes.header-2}} ║")
    print(f"║ {"а с самим собой. Готов показать, чего стоишь?":<{Sizes.header-2}} ║")
    print(f"╟{"":─^{Sizes.header}}╢")
    print(f"║{" Любой ввод для продолжения ":^{Sizes.header}}║")
    print(end)
...
def invalid_input():
    ...
    print(ac)
    print(division)
    print(f"╟{" Ошибка ":─^{Sizes.header}}╢")
    print(f"║{' """ Реплика фамильяра """    ':>{Sizes.header}}║") #в духе: "что произошло?"
    print(f"║{'Пожалуйста следи за вводом внимательнее.':^{Sizes.header}}║")
    print(f"╟{"":─^{Sizes.header}}╢")
    print(f"║{" Любой ввод для продолжения ":^{Sizes.header}}║")
    print(end)
    input("... --> ")
def critical_packet_loading_error():
    clear()
    print(head)
    print(ac)
    print(division)
    print(f"╟{" Критическая ошибка во время загрузки пакетов ":─^{Sizes.header}}╢")
    print(f"║{'Произошла критическая ошибка во время загрузки одного из пакетов приложения.':^{Sizes.header}}║")
    print(f"║{'Попробуйте переустановить приложение или заново скачать содержимое папки cache/*':^{Sizes.header}}║")
    print(f"║{'Если это не помогло, обратитесь за помощь напрямую к разработчику':^{Sizes.header}}║")
    print(f"║{" При любом вводе приложение завершит свою работу ":^{Sizes.header}}║")
    print(end)
    input("... --> ")
def user_packet_loading_error():
    clear()
    print(head)
    print(ac)
    print(division)
    print(f"╟{" Критическая ошибка во время загрузки пакетов пользователя ":─^{Sizes.header}}╢")
    print(f"║{'Произошла критическая ошибка во время загрузки одного из пакетов пользователя.':^{Sizes.header}}║")
    print(f"║{'Часть прогресса к сожалению оказалась безвозвратно потеряна.':^{Sizes.header}}║")
    print(f"║{'Что бы избежать повторения подобных ситуаций, просим связаться с разработчиком.':^{Sizes.header}}║")
    print(f"║{'Если ваш прогресс слишком велик свяжитесь с разработчиком для помощи в воссстановлении и введите save-exit':^{Sizes.header}}║")
    print(f"║{'Если же ваш прогресс не так важен вы можете восстановить файловую структуру: recovery':^{Sizes.header}}║")
    print(f"║{"Пожалуйста проверьте ввод перед продолжением":^{Sizes.header}}║")
    print(end)
def contact_information():
    ...
    print(ac)
    print(division)
    print(f"╟{" Связь с разработчком ":─^{Sizes.header}}╢")
    print(f"║{"Связаться со мной можно тут: tg: @Rataty1ch1K":^{Sizes.header}}║")
    print(f"║{"Или тут: ds: Rataty1ch1K":^{Sizes.header}}║")
    print(f"║{"Мой гитхаб: github.com/pacanciknorm78-crypto/self":^{Sizes.header}}║")
    print(f"╟{"":─^{Sizes.header}}╢")
    print(f"║{" Любой ввод для выхода ":^{Sizes.header}}║")
    print(end)
    input("... --> ")